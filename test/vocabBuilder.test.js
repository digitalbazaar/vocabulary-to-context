...
test('builds all vocabulary artifacts from template.html', async t => {
  ...
  await Promise.all([
    fs.writeFile(path.join(directory, 'vocabulary.yaml'), vocabularyYaml),
...
test('falls back to vocabulary.yml', async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'vocab-context-'));
  t.after(() => fs.rm(directory, {recursive: true, force: true}));
  await Promise.all([
    fs.writeFile(path.join(directory, 'vocabulary.yml'), vocabularyYaml),
    fs.writeFile(path.join(directory, 'template.html'), '<html></html>')
  ]);

  await buildVocab({baseDir: directory});

  expect((await fs.stat(path.join(directory, 'vocabulary.html'))).size)
    .to.be.greaterThan(0);
});

test('throws if the vocabulary file is missing', async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'vocab-context-'));
  t.after(() => fs.rm(directory, {recursive: true, force: true}));
  await fs.writeFile(path.join(directory, 'template.html'), '<html></html>');

  await assert.rejects(
    buildVocab({baseDir: directory}),
    /Missing vocabulary file/);
});

test('throws if template.html is missing', async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'vocab-context-'));
  t.after(() => fs.rm(directory, {recursive: true, force: true}));
  await fs.writeFile(
    path.join(directory, 'vocabulary.yaml'), vocabularyYaml);
